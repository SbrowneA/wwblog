...
//NAVIGATION BUTTONS
    ...
    function draftArticle(url) {
        if (confirm(`Are you sure you want to draft this post ${article.article_title})?
            \nLinked posts will not be drafted, only unliked.`)) {
            window.location.replace(url);
        } else {
            return false;
        }
    ...
    function setupButtons() {
         ...
         * gets the url from the value of the click event (i.e. the button.value) and forwards it to {@link deleteArticle()}
         */
        let draftArticleBtn = document.getElementById("draftArticleBtn");
        if (draftArticleBtn) {
            draftArticleBtn.addEventListener("click", () => {
                draftArticle(draftArticleBtn.value);
            });
        }
    }
